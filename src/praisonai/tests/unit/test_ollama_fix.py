...
def test_tool_call_parsing():
    ...
    assert name == "hello_world", f"Expected function name to be preserved, got '{name}'"
    assert args == {}, f"Expected empty dict, got {args}"
    print("✓ Error handling works")

    # Test 4: Recover from valid JSON prefix with trailing extra data
    tool_call_extra = {
        "id": "call_456",
        "function": {
            "name": "hello_world",
            "arguments": '{"name":"test"}{"unexpected":"tail"}'
        }
    }
    name, args, id = llm._parse_tool_call_arguments(tool_call_extra, is_ollama=False)
    assert name == "hello_world", f"Expected function name to be preserved, got '{name}'"
    assert args == {"name": "test"}, f"Expected parser to recover first JSON object, got {args}"
    print("✓ Extra-data recovery works")
