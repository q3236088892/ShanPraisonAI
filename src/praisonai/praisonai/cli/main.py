...
class PraisonAI:
    ...
    def _run_yaml_workflow(self, yaml_file: str, action_args: list, variables: dict = None, args=None):
        ...
        try:
            ...
            workflow_model_override = None
            workflow_max_tokens_override = None
            i = 0
            while i < len(action_args):
                if action_args[i] == "--var" and i + 1 < len(action_args):
                    ...
                    i += 2
                elif action_args[i] in ("--model", "-m", "--llm") and i + 1 < len(action_args):
                    workflow_model_override = action_args[i + 1].strip()
                    i += 2
                elif action_args[i] == "--max-tokens" and i + 1 < len(action_args):
                    try:
                        workflow_max_tokens_override = int(str(action_args[i + 1]).strip())
                    except Exception:
                        workflow_max_tokens_override = None
                    i += 2
                else:
            ...
            # Fallback from parsed args object
            if not workflow_model_override and args is not None:
                workflow_model_override = getattr(args, "llm", None) or getattr(args, "model", None)
            if workflow_max_tokens_override is None and args is not None:
                try:
                    workflow_max_tokens_override = int(getattr(args, "max_tokens", 16000))
                except Exception:
                    workflow_max_tokens_override = None
            
            # Bridge workflow model to YAML agent parser defaults
            # (YAML agents without explicit llm should honor workflow run -m / --llm)
            if workflow_model_override:
                import os
                os.environ["PRAISONAI_MODEL"] = workflow_model_override
            if workflow_max_tokens_override and workflow_max_tokens_override > 0:
                import os
                # Bridge max_tokens to YAML parser / workflow agents for CLI parity.
                # Direct prompt path already applies --max-tokens (default 16000);
                # keep workflow path consistent to avoid provider-default empty outputs.
                os.environ["PRAISONAI_WORKFLOW_MAX_TOKENS"] = str(workflow_max_tokens_override)
                os.environ["PRAISONAI_MAX_TOKENS"] = str(workflow_max_tokens_override)
            ...
            if workflow_model_override:
                print(f"[cyan]Workflow model override: {workflow_model_override}[/cyan]")
            if workflow_max_tokens_override and workflow_max_tokens_override > 0:
                print(f"[cyan]Workflow max_tokens override: {workflow_max_tokens_override}[/cyan]")
            ...
            if tools_file.exists():
                try:
                    ...
                    # Build registry from public functions defined in this tools.py
                    import inspect
                    for name, obj in vars(tools_module).items():
                        if name.startswith('_'):
                            continue
                        if inspect.isfunction(obj) and getattr(obj, "__module__", "") == tools_module.__name__:
