...
# Windows non-UTF8 consoles (e.g., CP936/GBK) can raise UnicodeEncodeError
# when printing status symbols. Prefer replacement over hard-fail in CLI runs.
try:
...
_EMPTY_RESPONSE_FALLBACK_MARKERS = (
    "模型返回空响应",
...
class AgentFlow:
    """
    A complete workflow with multiple steps (deterministic pipeline).
    
    AgentFlow-centric API with consolidated feature parameters.
    ...
    Note:
        The class was renamed from `Workflow` to `AgentFlow` in v1.0.
        `Workflow` and `Pipeline` remain as silent aliases for backward compatibility.
    """
    name: str = "AgentFlow"
    description: str = ""
    ...
    def _is_empty_response_fallback_output(self, output: Any) -> bool:
        """Detect built-in empty-response fallback messages from LLM layer."""
        if not isinstance(output, str):
            return False
        text = output.strip()
        if not text:
            return False
        return any(marker in text for marker in _EMPTY_RESPONSE_FALLBACK_MARKERS)

    def _compact_previous_output_for_prompt(
        self,
        previous_output: Any,
        *,
        step_name: str,
        ...
        """
        Compact large previous_output before injecting into next-step prompts.

        This avoids excessive prompt size in multi-step workflows (especially
        after parallel branches), which can lead to empty model responses on
        ...
        try:
            max_chars = int(os.getenv("PRAISONAI_WORKFLOW_CONTEXT_MAX_CHARS", "12000"))
        except Exception:
            max_chars = 12000
        ...
        tail = max_chars - head
        marker = "\n\n[... previous output truncated for workflow context ...]\n\n"
        compacted = text[:head] + marker + text[-tail:]

        if verbose:
            print(
                f"📦 Compacted previous_output for {step_name}: "
                f"{len(text)} -> {len(compacted)} chars"
            )
        return compacted
    ...
    def _build_non_llm_fallback_output(
        ...
        *,
        ...
        """
        Build deterministic fallback output from existing context when LLM returns
        empty-response fallback repeatedly.

        This keeps workflow steps (especially synthesis/file-output steps) from
        hard-failing due transient/provider-specific empty responses.
        """
        step_name = getattr(step, "name", "") or "step"
        step_name_lower = step_name.lower()
        base_text = str(previous_output).strip() if previous_output is not None else ""

        compacted = ""
        if base_text:
            compacted = self._compact_previous_output_for_prompt(
                base_text,
                step_name=f"{step_name}_fallback",
                verbose=verbose,
            )
        # Remove known empty-response marker lines so fallback output is not
        # re-detected as LLM fallback content in later checks / file writing.
        try:
            filtered_lines = []
            for line in compacted.splitlines():
                ...
                if stripped in {"---", "..."}:
                    continue
                if any(marker in line for marker in _EMPTY_RESPONSE_FALLBACK_MARKERS):
                    continue
                filtered_lines.append(line)
            compacted_clean = "\n".join(filtered_lines).strip()
        except Exception:
            compacted_clean = compacted.strip()
        if not compacted_clean:
            compacted_clean = (
                "No reliable model text was returned; "
                "using deterministic fallback based on previous workflow context."
            )

        # Generic branch fallback: derive lightweight snapshot from tools when
        # previous_output is unavailable or not useful.
        try:
            ...
            for t in tools:
                if callable(t) and getattr(t, "__name__", "") == "list_repo_dirs":
                    list_dirs_tool = t
                elif callable(t) and getattr(t, "__name__", "") == "list_repo_files":
                    list_files_tool = t

            if list_dirs_tool or list_files_tool:
                repo_path = str(all_variables.get("repo_path", "."))
                dirs_text = ""
                files_text = ""
                if list_dirs_tool:
                    dirs_text = str(list_dirs_tool(directory=repo_path, max_items=40) or "").strip()
                if list_files_tool:
                    files_text = str(list_files_tool(directory=repo_path, max_items=60) or "").strip()

                if dirs_text or files_text:
                    if "repository_map" not in step_name_lower and "project_analysis" not in step_name_lower:
                        return (
                            f"## {step_name} (Deterministic Branch Fallback)\n\n"
                            "### Directories Snapshot\n\n"
                            "```text\n"
                            f"{dirs_text or '[NO_DIR_DATA]'}\n"
                            "```\n\n"
                            "### Files Snapshot\n\n"
                            "```text\n"
                            f"{files_text or '[NO_FILE_DATA]'}\n"
                            "```\n"
                        )
        except Exception as e:
            logger.debug(f"Failed to build generic branch fallback from tools: {e}")

        # Try generating a lightweight repo map from local tools for map-like steps.
        if "repository_map" in step_name_lower or "repo_map" in step_name_lower:
            try:
                agent = getattr(step, "agent", None)
                tools = getattr(agent, "tools", None) or []
                list_dirs_tool = None
                ...
                if list_dirs_tool:
                    repo_path = str(all_variables.get("repo_path", "."))
                    dirs = list_dirs_tool(directory=repo_path, max_items=200)
                    if isinstance(dirs, str):
                        dirs = dirs.strip()
                        if dirs and not dirs.startswith("[ERROR]"):
                            return (
                                ...
                                "```\n\n"
                                "## Notes\n\n"
                                "- Generated via deterministic workflow fallback.\n"
                            )
            except Exception as e:
                logger.debug(f"Failed to build repository-map fallback from tools: {e}")

        # Try generating a lightweight project analysis snapshot from local tools.
        if "project_analysis" in step_name_lower or ("analysis" in step_name_lower and getattr(step, "output_file", None)):
            try:
                agent = getattr(step, "agent", None)
                tools = getattr(agent, "tools", None) or []
                list_dirs_tool = None
                ...
                files_text = ""
                if list_dirs_tool:
                    dirs_text = str(list_dirs_tool(directory=repo_path, max_items=120) or "").strip()
                if list_files_tool:
                    files_text = str(list_files_tool(directory=repo_path, max_items=160) or "").strip()

                if dirs_text or files_text:
                    return (
                        "# Project Analysis (Deterministic Fallback)\n\n"
                        "## Summary\n\n"
                        "- Model returned empty output repeatedly in synthesis stage.\n"
                        "- This report is generated from local repository tool outputs.\n\n"
                        "## Top Directories\n\n"
                        "```text\n"
                        f"{dirs_text or '[NO_DIR_DATA]'}\n"
                        "```\n\n"
                        ...
                        "```\n\n"
                        "## Notes\n\n"
                        "- Re-run after gateway/model stabilizes for richer semantic analysis.\n"
                    )
            except Exception as e:
                logger.debug(f"Failed to build project-analysis fallback from tools: {e}")

        # For synthesis / variable-persist steps, return compacted upstream context.
        if getattr(step, "output_variable", None):
            return compacted_clean

        # For file-output steps, ensure markdown-friendly fallback body.
        if getattr(step, "output_file", None):
            return (
                "# Workflow Output (Fallback)\n\n"
                "The model returned empty output repeatedly. "
                "Using deterministic fallback content from previous workflow step.\n\n"
                f"{compacted_clean}"
            )

        return compacted_clean

    def _build_minimal_persisted_fallback_output(
        self,
        step: Any,
        previous_output: Any,
        *,
        ...
        """
        Build a guaranteed non-empty fallback for persisted workflow steps.

        This is the last safety net when deterministic fallback cannot be
        constructed from previous context.
        """
        step_name = getattr(step, "name", "") or "step"
        header = (
            "# Workflow Output (Safety Fallback)\n\n"
            f"- Step: `{step_name}`\n"
            f"- Reason: {reason}\n"
            "- The step persisted this fallback to avoid workflow interruption.\n"
        )
        ...
        if previous_output is not None:
            try:
                compacted = self._compact_previous_output_for_prompt(
                    previous_output,
                    step_name=f"{step_name}_minimal_fallback",
                    verbose=verbose,
                )
                compacted = str(compacted or "").strip()
                if compacted:
                    snapshot = (
        ...
        if snapshot:
            return f"{header}{snapshot}"
        return (
            f"{header}\n"
            "No previous context was available at this stage.\n"
        )

    def _build_repo_snapshot_from_agent_tools(
        self,
        agent: Any,
        all_variables: Dict[str, Any],
        *,
        ...
        """
        Build a compact repository snapshot by directly invoking local file tools.

        This helps workflows produce useful answers even when model-side tool calls
        are unstable on some gateways.
        """
        if agent is None:
            return ""
        tools = getattr(agent, "tools", None) or []
        ...
        if not list_dirs_tool and not list_files_tool:
            return ""
        ...
        try:
            if list_dirs_tool:
                dirs_text = str(list_dirs_tool(directory=repo_path, max_items=30) or "").strip()
            if list_files_tool:
                files_text = str(list_files_tool(directory=repo_path, max_items=40) or "").strip()
        except Exception as e:
            logger.debug(f"Failed to build repo snapshot from tools: {e}")
            return ""

        if not dirs_text and not files_text:
            return ""

        snapshot = (
            "Repository snapshot (pre-collected):\n"
            "```text\n"
            "[Directories]\n"
            f"{dirs_text or '[NO_DIR_DATA]'}\n\n"
            "[Files]\n"
        ...
        if verbose:
            print("📎 Injected local repository snapshot into branch prompt.")
        return snapshot
    ...
    def run(
        ...
        while i < len(self.steps):
            ...
            while retry_count <= max_retries:
                ...
                try:
                    if step.handler:
                        ...
                        # Preserve YAML parser auto-action semantics for Task-wrapped agent steps
                        if action == "__AUTO_ACTION__":
                            action = "{{previous_output}}" if index > 0 else "{{input}}"
                        for key, value in all_variables.items():
                            action = action.replace(f"{{{{{key}}}}}", str(value))
                        if previous_output:
                            previous_context = self._compact_previous_output_for_prompt(
                                previous_output,
                                step_name=step.name,
                                verbose=verbose,
                            )
                            if "{{previous_output}}" in action:
                                action = action.replace("{{previous_output}}", previous_context)
                            else:
                        ...
                        if agent_class_name in ('AudioAgent', 'VideoAgent', 'ImageAgent', 'OCRAgent', 'DeepResearchAgent'):
                            ...
                            # Support both normalized private fields and Task public fields.
                            step_output_json = getattr(step, '_output_json', None) or getattr(step, 'output_json', None)
                            step_output_pydantic = getattr(step, '_output_pydantic', None) or getattr(step, 'output_pydantic', None)
                ...
                # Empty-response fallback from LLM layer should not be treated as success.
                if self._is_empty_response_fallback_output(output):
                    if retry_count < max_retries:
                        retry_count += 1
                        validation_feedback = (
                            "Previous model reply was an empty-response fallback message. "
                            "Please return concrete final content."
                        )
                        if verbose:
                            print(
                                f"⚠️ {step.name} produced empty-response fallback "
                                f"(attempt {retry_count}/{max_retries}), retrying..."
                            )
                        continue
                    deterministic_fallback = self._build_non_llm_fallback_output(
                        step,
                        previous_output,
                        all_variables,
                        verbose=verbose,
                    ...
                    if deterministic_fallback:
                        ...
                        if verbose:
                            print(
                                f"⚠️ {step.name} used deterministic fallback output "
                                "after repeated empty-response."
                            )
                    else:
                        requires_persisted_output = bool(
                            getattr(step, "output_file", None)
                            or getattr(step, "output_variable", None)
                        )
                        if requires_persisted_output:
                            output = self._build_minimal_persisted_fallback_output(
                                step,
                                previous_output,
                                reason="empty-response fallback after retries",
                                verbose=verbose,
                            )
                            step_error = None
                            if verbose:
                                print(
                                    f"⚠️ {step.name} used safety fallback output "
                                    "after repeated empty-response."
                                )
                        else:
                            step_error = RuntimeError(
                                "Model returned empty-response fallback message after retries"
                            )

                # For steps expected to produce persisted output, empty text is also a failure.
                expects_persisted_output = bool(
                    getattr(step, 'output_file', None) or getattr(step, 'output_variable', None)
                )
                empty_text_output = isinstance(output, str) and not output.strip()
                if expects_persisted_output and empty_text_output and not step_error:
                    if retry_count < max_retries:
                        retry_count += 1
                        validation_feedback = (
                            "Previous model reply was empty. Please provide concrete final content."
                        )
                        if verbose:
                            print(
                                f"⚠️ {step.name} produced empty output "
                                f"(attempt {retry_count}/{max_retries}), retrying..."
                            )
                        continue
                    deterministic_fallback = self._build_non_llm_fallback_output(
                    ...
                    if deterministic_fallback:
                        ...
                        if requires_persisted_output:
                            output = self._build_minimal_persisted_fallback_output(
                                step,
                                previous_output,
                                reason="empty output after retries",
                                verbose=verbose,
                            )
                            step_error = None
                            if verbose:
                            ...
                            step_error = RuntimeError("Model returned empty output after retries")
            ...
            # NOTE:
            # - Save when output is an empty string (""), because that's still a valid model response.
            # - Only skip when output is truly None.
            ...
            elif hasattr(step, 'output_file') and step.output_file and skip_file_write_for_fallback and verbose:
                print(f"⚠️ Skipped writing fallback content to file for step: {step.name}")
            ...
            if verbose:
                step_status = self.step_statuses.get(step.name, "completed")
                prefix = "✅" if step_status == "completed" else "❌"
                print(f"{prefix} {step.name}: {str(output)}")
            
            # Fail fast on step errors to avoid cascading retries/noisy follow-up failures.
            if step_error:
                if verbose:
                    print(f"🛑 Stopping workflow due to failed step: {step.name}")
                break
        ...
        # Update workflow status
        failed_step_names = [name for name, status in self.step_statuses.items() if status == "failed"]
        self.status = "failed" if failed_step_names else "completed"
        ...
        if failed_step_names:
            final_result["error"] = f"Workflow has {len(failed_step_names)} failed step(s): {', '.join(failed_step_names)}"
    ...
    def _execute_single_step_internal(
        ...
        if normalized.handler:
            ...
            try:
                ...
                # Retry empty/fallback outputs at branch level, otherwise parallel
                # stages can pass only fallback markers to synthesis.
                raw_max_retries = getattr(normalized, "max_retries", 1)
                try:
                    branch_retries = int(raw_max_retries if raw_max_retries is not None else 1)
                except Exception:
                    branch_retries = 1
                branch_retries = max(1, branch_retries)

                raw_retry_delay = getattr(normalized, "retry_delay", 1.0)
                try:
                    branch_retry_delay = float(raw_retry_delay if raw_retry_delay is not None else 1.0)
                except Exception:
                    branch_retry_delay = 1.0
                branch_retry_delay = max(0.0, branch_retry_delay)

                validation_feedback = ""
                precollected_snapshot = self._build_repo_snapshot_from_agent_tools(
                    normalized.agent,
                    all_variables,
                    verbose=verbose,
                )
                for attempt in range(branch_retries + 1):
                    ...
                    if previous_output:
                        ...
                        if "{{previous_output}}" in action:
                            action = action.replace("{{previous_output}}", str(previous_context))
                        else:
                            # Auto-append context if not explicitly referenced
                            action = f"{action}\n\nContext from previous step:\n{previous_context}"
                    if precollected_snapshot:
                        action = (
                            f"{action}\n\n"
                            "Use the pre-collected repository snapshot below as primary evidence. "
                            "No tool call is required for this response.\n\n"
                            f"{precollected_snapshot}"
                        )
                    action = action.replace("{{input}}", input)
                    if validation_feedback:
                        action = f"{action}\n\nPrevious attempt feedback: {validation_feedback}"
                    if verbose:
                        print(
                            f"[STEP] {normalized.name} attempt {attempt + 1}/{branch_retries + 1} calling model..."
                        )
                    ...
                    if agent_class_name in ('AudioAgent', 'VideoAgent', 'ImageAgent', 'OCRAgent', 'DeepResearchAgent'):
                        ...
                        # Keep behavior aligned with sequential workflow execution:
                        # pass YAML tool_choice / structured output / attachments when present.
                        step_attachments = getattr(normalized, 'images', None)
                        if not step_attachments:
                            image_var = (
                                all_variables.get('image_path')
                                or all_variables.get('image_url')
                                or all_variables.get('image')
                            )
                            if image_var:
                                step_attachments = [image_var] if isinstance(image_var, str) else image_var

                        step_output_json = getattr(normalized, '_output_json', None) or getattr(normalized, 'output_json', None)
                        step_output_pydantic = getattr(normalized, '_output_pydantic', None) or getattr(normalized, 'output_pydantic', None)
                        ...
                        yaml_tool_choice = getattr(normalized.agent, '_yaml_tool_choice', None)
                        if yaml_tool_choice:
                            chat_kwargs["tool_choice"] = yaml_tool_choice

                        output = normalized.agent.chat(action, **chat_kwargs)
                    ...
                    is_fallback = self._is_empty_response_fallback_output(output)
                    is_empty_text = isinstance(output, str) and not output.strip()
                    if not is_fallback and not is_empty_text:
                        break

                    if attempt < branch_retries:
                        validation_feedback = (
                            "Previous model reply was empty/fallback. "
                            "Please provide concrete final content."
                        )
                        if verbose:
                            print(
                                f"[WARN] {normalized.name} branch empty/fallback "
                                f"(attempt {attempt + 1}/{branch_retries}), retrying..."
                            )
                        if branch_retry_delay > 0:
                            import time as _time
                            _time.sleep(min(branch_retry_delay, 3.0))
                        continue
                    ...
                    if deterministic_fallback:
                        ...
                        output = (
                            f"[WARN] Step {normalized.name} returned empty output after retries."
                        )
            except Exception as e:
                output = f"Error: {e}"
        ...
        if verbose:
            print(f"[STEP] {normalized.name}: {str(output)}")
...
CONCISE SUMMARY:"""
    ...
    def _execute_parallel(
        ...
        if verbose:
            print(f"[PARALLEL] Running {len(parallel_step.steps)} steps in parallel...")
        ...
        # IMPORTANT: Limit max_workers to prevent rate limit issues.
        # Default cap is 3, overridable via:
        #   PRAISONAI_WORKFLOW_PARALLEL_MAX_WORKERS
        # (set to 1 for serial A/B debugging on unstable gateways)
        from ..trace.context_events import copy_context_to_callable, get_context_emitter

        default_cap = 3
        try:
            configured_cap = int(
                os.getenv("PRAISONAI_WORKFLOW_PARALLEL_MAX_WORKERS", str(default_cap))
            )
        except Exception:
            configured_cap = default_cap
        if configured_cap <= 0:
            configured_cap = 1
        effective_workers = min(configured_cap, len(parallel_step.steps))
        if verbose:
            print(f"[PARALLEL] workers: {effective_workers} (cap={configured_cap})")

        # IMPORTANT:
        # For some third-party gateways, LLM calls from worker threads can become
        # unstable (empty responses). When worker cap is 1, run strictly in the
        # current thread to avoid thread-related variability.
        if effective_workers <= 1:
            for idx, step in enumerate(parallel_step.steps):
                ...
                try:
                    step_result = self._execute_single_step_internal(
                        step, optimized_previous, input, all_variables.copy(), model, verbose, idx, stream, depth=depth+1
                    )
                    results.append({"step": step_result["step"], "output": step_result["output"]})
                    ...
                    def execute_with_branch(step=step, idx=idx, opt_prev=optimized_previous):
                        ...
                        try:
                            return self._execute_single_step_internal(
                                step, opt_prev, input, all_variables.copy(), model, verbose, idx, stream, depth=depth+1
                            )
                        finally:
                            emitter.clear_branch()
