...
class YAMLWorkflowParser:
    ...
    def parse_file(self, file_path: Union[str, Path], extra_vars: Optional[Dict[str, Any]] = None) -> Workflow:
        ...
        # Always prefer UTF-8 for workflow files to avoid Windows locale
        # decode issues (e.g., GBK on zh-CN systems).
        last_error = None
        yaml_content = None
        for enc in ("utf-8", "utf-8-sig"):
            try:
                yaml_content = file_path.read_text(encoding=enc)
                break
            except UnicodeDecodeError as e:
                last_error = e
                continue
        if yaml_content is None:
            # Final compatibility fallback: use system preferred encoding with
            # replacement to keep parser resilient for legacy non-UTF8 files.
            sys_enc = locale.getpreferredencoding(False) or "utf-8"
            try:
                yaml_content = file_path.read_text(encoding=sys_enc, errors="replace")
            except Exception:
                if last_error:
                    raise last_error
                raise
    ...
    def _parse_workflow_data(self, data: Dict[str, Any], extra_vars: Optional[Dict[str, Any]] = None) -> Workflow:
        ...
        # Build planning config.
        # Keep `reasoning` from YAML even when planning_llm is not explicitly set.
        planning_value = planning
        if planning_llm or reasoning:
            from .workflow_configs import WorkflowPlanningConfig
            planning_value = WorkflowPlanningConfig(
                enabled=bool(planning),
                llm=planning_llm,
    ...
    def _create_agent(self, agent_id: str, config: Dict[str, Any], workflow_llm: Optional[Any] = None) -> Any:
        ...
        # Optional timeout override for workflow agent LLM calls.
        # Priority: agent config timeout > env PRAISONAI_WORKFLOW_LLM_TIMEOUT_SEC > env PRAISONAI_LLM_TIMEOUT
        llm_timeout = _coerce_positive_int(config.get('timeout'))
        if llm_timeout is None:
            env_timeout = (
                os.getenv('PRAISONAI_WORKFLOW_LLM_TIMEOUT_SEC')
                or os.getenv('PRAISONAI_LLM_TIMEOUT')
            )
            if env_timeout:
                llm_timeout = _coerce_positive_int(env_timeout)

        # Optional max_tokens override for workflow agent LLM calls.
        # Priority:
        # 1) agent.max_tokens in YAML
        # 2) agent.llm.max_tokens in YAML
        # 3) PRAISONAI_WORKFLOW_MAX_TOKENS
        # 4) PRAISONAI_MAX_TOKENS
        llm_max_tokens = _coerce_positive_int(config.get('max_tokens'))
        if llm_max_tokens is None and isinstance(config.get('llm'), dict):
            llm_max_tokens = _coerce_positive_int(config.get('llm', {}).get('max_tokens'))
        if llm_max_tokens is None:
            env_max_tokens = (
                os.getenv('PRAISONAI_WORKFLOW_MAX_TOKENS')
                or os.getenv('PRAISONAI_MAX_TOKENS')
            )
            llm_max_tokens = _coerce_positive_int(env_max_tokens)

        if (llm_timeout is not None or llm_max_tokens is not None) and llm:
            if isinstance(llm, str):
                llm = {"model": llm}
            elif isinstance(llm, dict):
                llm = llm.copy()
            if llm_timeout is not None and llm.get("timeout") is None:
                llm["timeout"] = llm_timeout
            if llm_max_tokens is not None:
                existing_max_tokens = _coerce_positive_int(llm.get("max_tokens"))
    ...
    def _parse_agent_step(self, step_data: Dict) -> Any:
        """
        Parse an agent step.
        ...
        if agent_id in self._agents:
            ...
            # IMPORTANT:
            # Step-level config must not be stored on shared Agent instances.
            # Reusing the same agent in multiple YAML steps would cause later
            # step config (e.g., output_file) to overwrite earlier steps.
            step_level_fields = {
                "name",
                "action",
                "guardrail",
            ...
            has_step_overrides = any(field in step_data for field in step_level_fields)

            # Backward-compatible fast path: plain agent reference without overrides
            if not has_step_overrides:
                return agent

            task_kwargs: Dict[str, Any] = {
                "name": step_data.get("name", agent_id),
                "agent": agent,
                "tools": getattr(agent, "tools", None),
                # Preserve runtime default action behavior in workflow executor.
                "action": step_data.get("action", "__AUTO_ACTION__"),
            }

            if "guardrail" in step_data:
                task_kwargs["guardrails"] = step_data.get("guardrail")
            if "max_retries" in step_data:
                task_kwargs["max_retries"] = step_data.get("max_retries")
            if "expected_output" in step_data:
                task_kwargs["expected_output"] = step_data.get("expected_output")
            if "context" in step_data:
                task_kwargs["context"] = step_data.get("context")
            if "output_file" in step_data:
                task_kwargs["output_file"] = step_data.get("output_file")
            if "output_json" in step_data:
                task_kwargs["output_json"] = step_data.get("output_json")
            if "output_pydantic" in step_data:
                task_kwargs["output_pydantic"] = step_data.get("output_pydantic")
            if "create_directory" in step_data:
                task_kwargs["create_directory"] = step_data.get("create_directory")
            if "callback" in step_data:
                task_kwargs["callback"] = step_data.get("callback")
            if "async_execution" in step_data:
                task_kwargs["async_execution"] = step_data.get("async_execution")
            if "output_variable" in step_data:
                task_kwargs["output_variable"] = step_data.get("output_variable")
            if "skip_on_failure" in step_data:
                task_kwargs["skip_on_failure"] = step_data.get("skip_on_failure")
            if "retry_delay" in step_data:
                task_kwargs["retry_delay"] = step_data.get("retry_delay")
    ...
    def _parse_parallel_step(self, step_data: Dict) -> Dict:
        ...
        for item in parallel_config:
            if not isinstance(item, dict):
                continue
            if 'agent' in item:
                # Reuse agent-step parser so each parallel branch gets its own
                # step wrapper (Task) instead of mutating shared Agent objects.
                # This prevents branch action/config leakage across steps.
                parsed_step = self._parse_agent_step(item)
                if parsed_step is not None:
                    parallel_steps.append(parsed_step)
