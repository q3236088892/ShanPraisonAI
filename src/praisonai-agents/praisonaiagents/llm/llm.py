...
from typing import Any, Dict, List, Optional, Union, Literal, Callable, TYPE_CHECKING, Type
...
{{"name": "tool_name", "arguments": {{"arg1": value1, "arg2": value2}}}}"""
    ...
    def _get_effective_openai_base_url(self) -> str:
        """Return configured OpenAI-compatible base URL from explicit config or env vars."""
        return (self.base_url or os.getenv("OPENAI_API_BASE") or os.getenv("OPENAI_BASE_URL") or "").strip()

    def _is_official_openai_base_url(self, base_url: str) -> bool:
        """Check whether a base URL points to official OpenAI/Azure OpenAI endpoints."""
        normalized = (base_url or "").strip().lower()
        if not normalized:
            return False
        return ("api.openai.com" in normalized) or ("openai.azure.com" in normalized)

    def _is_openai_family_model(self) -> bool:
        """Detect OpenAI-family model names (with or without provider prefix)."""
        if not self.model:
            return False

        model_lower = self.model.lower()
        if "/" in model_lower:
            provider, remainder = model_lower.split("/", 1)
            if provider not in ("openai", "azure", "responses"):
                return False
            model_lower = remainder

        return model_lower.startswith(("gpt-", "o1", "o3", "o4", "chatgpt-"))

    def _should_force_custom_openai_provider(self) -> bool:
        """
        Use custom_openai provider for non-official OpenAI-compatible endpoints.

        This avoids LiteLLM responses-bridge auto-routing for some models (e.g. gpt-5.*),
        which can fail on third-party endpoints that only implement chat/completions.
        """
        base_url = self._get_effective_openai_base_url()
        if not base_url:
            return False
        if self._is_official_openai_base_url(base_url):
            return False
        if not self._is_openai_family_model():
            return False

        model_lower = (self.model or "").lower()
        return not model_lower.startswith(("custom_openai/", "openai_like/", "vllm/", "hosted_vllm/"))

    def _get_model_for_litellm_completion(self) -> str:
        """Get model name used for litellm.completion(), with provider rewrite when needed."""
        if not self._should_force_custom_openai_provider():
            return self.model

        model_name = self.model
        if "/" in model_name:
            provider, remainder = model_name.split("/", 1)
            if provider.lower() in ("openai", "azure", "responses"):
                model_name = remainder

        rewritten = f"custom_openai/{model_name}"
        logging.debug(
            "[LITELLM_COMPAT] Rewriting model '%s' -> '%s' for custom OpenAI-compatible endpoint",
            self.model,
            rewritten,
        )
        return rewritten

    def _coerce_content_to_text(self, content: Any, preserve_whitespace: bool = False) -> str:
        """Best-effort extraction of textual content from provider-specific content shapes."""
        if content is None:
            return ""
        ...
        if isinstance(content, dict):
            ...
            if isinstance(text_candidate, str):
                return text_candidate if preserve_whitespace else text_candidate.strip()
            return ""
        if isinstance(content, list):
            parts: List[str] = []
            for item in content:
                if isinstance(item, str):
                    s = item if preserve_whitespace else item.strip()
                    if s:
                        parts.append(s)
                    continue
                if isinstance(item, dict):
                    # OpenAI-style blocks: {"type":"output_text","text":"..."}
                    text_candidate = item.get("text")
                    if not text_candidate and isinstance(item.get("content"), str):
                        text_candidate = item.get("content")
                    if not text_candidate and isinstance(item.get("output_text"), str):
                        text_candidate = item.get("output_text")
                    if isinstance(text_candidate, str):
                        s = text_candidate if preserve_whitespace else text_candidate.strip()
                        if s:
                            parts.append(s)
                    continue
                item_dict = self._to_mapping(item)
                if isinstance(item_dict, dict):
                    text_candidate = (
                        item_dict.get("text")
                        or item_dict.get("content")
                        or item_dict.get("output_text")
                        or item_dict.get("final_text")
                        or item_dict.get("reasoning_content")
                        or item_dict.get("reasoning")
                        or item_dict.get("answer")
                        or item_dict.get("output")
                    )
                    if isinstance(text_candidate, str):
                        s = text_candidate if preserve_whitespace else text_candidate.strip()
                        if s:
                            parts.append(s)
                    continue
                for attr in ("text", "content", "output_text", "final_text", "reasoning_content", "reasoning", "answer", "output"):
                    try:
                        v = getattr(item, attr, None)
                    except Exception:
                        v = None
                    s = self._coerce_content_to_text(v, preserve_whitespace=preserve_whitespace)
                    if s:
                        parts.append(s)
                        break
    ...
    def _to_mapping(self, obj: Any) -> Optional[Dict[str, Any]]:
        """Best-effort conversion of SDK response objects to dict-like mapping."""
    ...
    def _extract_text_from_message_payload(self, message: Any) -> str:
        """Extract answer text from a Chat Completions message payload."""
        if message is None:
            return ""

        candidates: List[Any] = []

        message_dict = self._to_mapping(message)

        if isinstance(message_dict, dict):
            candidates.extend([
                message_dict.get("content"),
                message_dict.get("text"),
                message_dict.get("output_text"),
                message_dict.get("final_text"),
                message_dict.get("reasoning_content"),
                message_dict.get("reasoning"),
                message_dict.get("answer"),
                message_dict.get("output"),
            ])
            provider_fields = message_dict.get("provider_specific_fields")
            if isinstance(provider_fields, dict):
                candidates.extend([
                    provider_fields.get("output_text"),
                    provider_fields.get("final_text"),
                    provider_fields.get("content"),
                    provider_fields.get("text"),
                    provider_fields.get("reasoning_content"),
                    provider_fields.get("reasoning"),
                    provider_fields.get("answer"),
                    provider_fields.get("output"),
                ])
        if message_dict is None or not isinstance(message_dict, dict):
            for attr in ("content", "text", "output_text", "final_text", "reasoning_content", "reasoning", "answer", "output"):
                try:
        ...
        for candidate in candidates:
            text = self._coerce_content_to_text(candidate)
            if text:
                return text
        return ""

    def _extract_text_from_completion_response(self, response: Any) -> str:
        """Extract answer text from a full Chat Completions style response object."""
        if response is None:
            return ""

        response_dict = self._to_mapping(response)
        if isinstance(response_dict, dict) and not isinstance(response, dict):
            recovered_text = self._extract_text_from_completion_response(response_dict)
            if recovered_text:
        ...
        if choices:
            ...
            first_choice_dict = self._to_mapping(first_choice)
            if isinstance(first_choice_dict, dict):
                message_payload = first_choice_dict.get("message")
                text = self._extract_text_from_message_payload(message_payload)
                if text:
                    return text
                choice_provider_fields = first_choice_dict.get("provider_specific_fields")
                if isinstance(choice_provider_fields, dict):
                    for key in (
                        ...
                        text = self._coerce_content_to_text(choice_provider_fields.get(key))
                        if text:
                            return text
                for key in ("text", "output_text", "content"):
                    text = self._coerce_content_to_text(first_choice_dict.get(key))
                    if text:
                        return text
                for key in ("final_text", "reasoning_content", "reasoning", "answer", "output"):
                    text = self._coerce_content_to_text(first_choice_dict.get(key))
                    if text:
                        return text
                text = self._coerce_content_to_text(first_choice_dict.get("delta"))
                if text:
                    return text
            else:
                ...
                if text:
                    return text
                for key in ("text", "output_text", "content", "delta", "final_text", "reasoning_content", "reasoning", "answer", "output"):
                    text = self._coerce_content_to_text(getattr(first_choice, key, None))
                    if text:
                        return text

        # Top-level fallback for provider-specific shapes
        top_level_candidates: List[Any] = []
        if isinstance(response, dict):
            top_level_candidates.extend([
                ...
                response.get("answer"),
                response.get("provider_specific_fields"),
            ])
        else:
            for key in ("text", "output_text", "content", "output", "response", "message", "final_text", "reasoning_content", "reasoning", "answer", "provider_specific_fields"):
                top_level_candidates.append(getattr(response, key, None))
    ...
    def _get_empty_response_fallback_message(self) -> str:
        """Human-readable fallback message when model returns an empty response."""
        effective_base = self._get_effective_openai_base_url()
        if effective_base and not self._is_official_openai_base_url(effective_base):
            return (
                "模型返回空响应。检测到当前使用第三方 OpenAI 兼容网关，"
                "当前轮未提取到最终文本。"
                "可能原因包括：网关返回字段差异、步骤未稳定触发 tool call、或会话瞬时空包。"
                "请重试，或确保该步骤至少触发一次工具调用后再生成最终内容。"
            )
        return "模型返回空响应，请重试一次。"

    def _get_empty_response_fallback_model(self) -> Optional[str]:
        """
        Optional model fallback for third-party OpenAI-compatible gateways.

        Some gateways return empty text for specific models (notably gpt-5.*) in
        no-tools chat/completions scenarios. When detected, we can retry once
        with a more compatible model.
        """
        effective_base = self._get_effective_openai_base_url()
        if not effective_base or self._is_official_openai_base_url(effective_base):
            return None

        env_fallback = (
            os.getenv("PRAISONAI_EMPTY_RESPONSE_FALLBACK_MODEL")
            or os.getenv("PRAISONAI_FALLBACK_MODEL")
            or ""
        ).strip()

        if env_fallback:
            fallback_model = env_fallback
        else:
            current_model_lower = (self.model or "").lower()
            # Default safeguard only for known problematic families on some gateways.
            if "gpt-5" not in current_model_lower and "codex" not in current_model_lower:
                return None
            fallback_model = "custom_openai/gpt-4o-mini"

        current_litellm_model = (self._get_model_for_litellm_completion() or "").strip().lower()
        if fallback_model.strip().lower() == current_litellm_model:
            return None

        return fallback_model

    def _generate_tool_results_summary(self, tool_results: List[Any]) -> Optional[str]:
        """Generate a readable fallback summary from tool results (provider-agnostic)."""
        if not tool_results:
            return None
        ...
        for result in tool_results:
            ...
            if isinstance(result, list) and result and isinstance(result[0], dict) and "error" in result[0]:
                error_messages.append(result[0].get("error", "Unknown error"))
                continue
        ...
        for result in valid_results:
            ...
            if not result_str:
                continue
            if not isinstance(result, (int, float)) and result_str[-1] not in ".!?":
                result_str += "."
            summary_parts.append(result_str)
        return " ".join(summary_parts).strip() if summary_parts else None

    def _retry_non_streaming_for_empty_response(
        self,
        *,
        response_text: str,
        stream: bool,
        ...
        temperature: float,
        output_json: Optional[Union[bool, BaseModel]],
        output_pydantic: Optional[Type[BaseModel]],
        extra_kwargs: Dict[str, Any],
    ) -> str:
        """One-shot non-streaming retry when first-turn streaming response is empty."""
        normalized = self._coerce_content_to_text(response_text)
        # Even when `stream=False` at API level, some verbose paths may still use
        # transient streaming for UX and can return empty text on third-party gateways.
        # So we retry once whenever first-turn text is empty.
        if normalized or iteration_count != 0:
            return normalized
        ...
        try:
            ...
            for attempt in range(1, max_attempts + 1):
                retry_resp = litellm.completion(
                    **self._build_completion_params(
                        ...
                        **{k: v for k, v in extra_kwargs.items() if k != "reasoning_steps"},
                    )
                )
                message = retry_resp["choices"][0]["message"] if retry_resp and retry_resp.get("choices") else {}
                recovered = self._extract_text_from_message_payload(message)
                if recovered:
                    logging.warning(
                        "[EMPTY_RESPONSE_FALLBACK] Recovered non-empty response via non-stream retry attempt %s/%s",
                        attempt,
                        max_attempts,
                    )
                    return recovered
            ...
            logging.warning(f"[EMPTY_RESPONSE_FALLBACK] Non-stream retry failed: {e}")

        # Last-resort fallback for no-tools scenarios on third-party gateways:
        # retry once with a compatibility model (configurable via env var).
        if not normalized and not formatted_tools:
            fallback_model = self._get_empty_response_fallback_model()
            if fallback_model:
                try:
                    ...
                    fallback_text = self._extract_text_from_completion_response(fallback_resp)
                    if fallback_text:
                        logging.warning(
                            "[EMPTY_RESPONSE_FALLBACK_MODEL] Recovered non-empty response via fallback model '%s'",
                            fallback_model,
                        )
                        return fallback_text
                except Exception as e:
                    logging.warning(
                        "[EMPTY_RESPONSE_FALLBACK_MODEL] Fallback model '%s' failed: %s",
                        fallback_model,
                        e,
                    )
    ...
    def _generate_ollama_tool_summary(self, tool_results: List[Any], response_text: str) -> Optional[str]:
        ...
        return self._generate_tool_results_summary(tool_results)
...
Now provide your final answer using this result. Summarize the information naturally for the user."""
    ...
    def _should_force_tool_usage(self, response_text: str, tool_calls: Optional[List], formatted_tools: Optional[List], iteration_count: int) -> bool:
        ...
        if self.force_tool_usage == 'never':
            ...
            # For non-Ollama providers (especially third-party OpenAI-compatible gateways),
            # repeatedly forcing tool calls can cause no-progress loops.
            # Degrade to normal behavior after one retry to avoid empty final responses.
            if not self._is_ollama_provider() and iteration_count >= 1:
                logging.warning(
                    "[FORCE_TOOL_USAGE] No tool call after forced retry; "
                    "degrading to normal response mode for provider compatibility."
                )
                return False
            return True
        elif self.force_tool_usage == 'auto':
            # Auto mode:
            # - Ollama on first iteration (existing behavior)
            # For non-Ollama providers, do not force tool usage in auto mode.
            # Forcing here can introduce no-progress loops on gateways that return
            # plain-text answers for tool-eligible prompts.
            if iteration_count != 0:
                return False
            return self._is_ollama_provider()
        return False
    ...
    def _process_stream_delta(self, delta, response_text: str, tool_calls: List[Dict], formatted_tools: Optional[List] = None) -> tuple:
        ...
        """
        # Process textual content (provider-specific delta fields).
        text_piece = ""
        try:
            text_piece = self._coerce_content_to_text(
        ...
        if not text_piece:
            for attr in ("text", "output_text", "delta", "final_text", "reasoning_content", "reasoning", "answer", "output"):
                try:
                    candidate = getattr(delta, attr, None)
                except Exception:
                    candidate = None
                text_piece = self._coerce_content_to_text(
                    candidate,
        ...
        if not text_piece and isinstance(delta, dict):
            for key in ("content", "text", "output_text", "delta", "final_text", "reasoning_content", "reasoning", "answer", "output"):
                text_piece = self._coerce_content_to_text(
                    delta.get(key),
                    preserve_whitespace=True,
                )
        ...
        # Capture tool calls from streaming chunks whenever present.
        # Do NOT gate on `_supports_streaming_tools()` here:
        # some third-party OpenAI-compatible gateways still emit tool_calls
        # in streaming deltas even if capability checks return False.
        if formatted_tools and hasattr(delta, 'tool_calls') and delta.tool_calls:
        ...
        # Legacy OpenAI-compatible providers may stream function_call instead of tool_calls
        # Example shape: delta.function_call = {"name": "...", "arguments": "..."}
        if formatted_tools and hasattr(delta, 'function_call') and getattr(delta, 'function_call'):
            fc = getattr(delta, 'function_call')
            if not tool_calls:
                tool_calls.append({
                    "id": f"legacy_fc_stream_{int(time.time() * 1000)}",
                    "type": "function",
                    "function": {"name": "", "arguments": ""}
                })
            ...
            if fc_name:
                tool_calls[0]["function"]["name"] = fc_name
            if fc_args:
                tool_calls[0]["function"]["arguments"] += fc_args
        
        return response_text, tool_calls

    def _convert_legacy_function_call_to_tool_calls(self, message: Any, iteration_count: int) -> Optional[List[Dict[str, Any]]]:
        """
        Convert legacy `function_call` payload (OpenAI old format) into `tool_calls`.
        """
        if not message:
            return None
        ...
        return [{
            "id": f"legacy_fc_{iteration_count}",
            "type": "function",
            "function": {
                "name": name,
    ...
    def _parse_tool_call_json_arguments(self, arguments_raw: Any, function_name: str = "unknown_function") -> Dict[str, Any]:
        """Parse tool-call JSON arguments with tolerance for trailing extra data."""
        ...
        if not isinstance(arguments_raw, str):
            logging.error(
                "Tool '%s' arguments must be JSON string/object, got %s",
                function_name,
                type(arguments_raw).__name__,
            )
            return {}

        arguments_str = arguments_raw.strip()
        ...
        try:
            ...
            # Some providers occasionally append extra text after a valid JSON object.
            # Try to recover the first JSON value instead of discarding the whole tool call.
            try:
                parsed, end_idx = json.JSONDecoder().raw_decode(arguments_str)
                trailing = arguments_str[end_idx:].strip()
                if trailing:
                    logging.warning(
                        "Recovered tool args for '%s' with trailing extra data ignored: %s",
                        function_name,
                        trailing[:120],
                    )
            except json.JSONDecodeError:
                logging.error(f"Error parsing tool call arguments: {err}")
                return {}
        ...
        logging.error(
            "Tool '%s' arguments JSON must decode to an object, got %s",
            function_name,
        ...
        try:
            if is_ollama:
                ...
                if "function" in tool_call and isinstance(tool_call["function"], dict):
                    ...
                    arguments = self._parse_tool_call_json_arguments(arguments_str, function_name)
            ...
            ) or (tool_call.get("name", "unknown_function") if isinstance(tool_call, dict) else "unknown_function")
            arguments = {}
            tool_call_id = tool_call.get("id", f"tool_{id(tool_call)}") if isinstance(tool_call, dict) else f"tool_{id(tool_call)}"
    ...
    def get_response(
        ...
        **kwargs
        ...
        try:
                ...
                try:
                    ...
                    if self._supports_responses_api():
                        ...
                        if tool_calls and execute_tool_fn:
                            ...
                            response_text = self._retry_non_streaming_for_empty_response(
                            ...
                            if response_text and response_text.strip():
                                ...
                                tool_summary = self._generate_tool_results_summary(accumulated_tool_results)
                                final_response_text = tool_summary or self._get_empty_response_fallback_message()
                            break
                    ...
                    if reasoning_steps:
                        ...
                        message_payload = resp["choices"][0]["message"]
                        reasoning_content = message_payload.get("provider_specific_fields", {}).get("reasoning_content")
                        response_text = self._extract_text_from_completion_response(resp)
                        final_response = resp
                    ...
                    # Otherwise do the existing streaming approach
                        ...
                        logging.debug(
                            "[TOOL_STREAM_DECISION] stream_arg=%s use_streaming=%s has_tools=%s supports_streaming_tools=%s model=%s",
                            stream,
                            use_streaming,
                            bool(formatted_tools),
                            self._supports_streaming_tools() if formatted_tools else None,
                            self.model,
                        )
                        ...
                        if use_streaming:
                            ...
                            try:
                                ...
                                if self._is_streaming_error_recoverable(streaming_error):
                                    ...
                                    try:
                                        if verbose:
                                            # Recovery path is explicitly non-streaming; do not switch back
                                            # to stream=True in verbose mode.
                                            logging.debug(
                                                "[TOOL_STREAM_POLICY] recoverable streaming error fallback "
                                                "uses non-streaming completion in verbose mode"
                                            )
                                            with _get_live()(
                                                _get_display_functions()['display_generating']("", current_time),
                                                console=self.console,
                                                ...
                                                message_payload = final_response["choices"][0]["message"]
                                                response_text = self._extract_text_from_completion_response(final_response)
                                                tool_calls = (
                                                    message_payload.get("tool_calls", [])
                                                    if isinstance(message_payload, dict)
                                                    else (getattr(message_payload, "tool_calls", []) or [])
                                                )
                        ...
                        if not use_streaming and not fallback_completed:
                            ...
                            if verbose:
                                # Respect tool-streaming capability gate:
                                # if use_streaming=False (e.g., provider/gateway doesn't reliably
                                # stream tool calls), do NOT force stream=True for UX.
                                logging.debug(
                                    "[TOOL_STREAM_POLICY] use_streaming=False in verbose mode; "
                                    "executing non-streaming completion for reliability"
                                )
                                with _get_live()(
                                    _get_display_functions()['display_generating']("", current_time),
                                    console=self.console,
                            ...
                            # Recovery for third-party OpenAI-compatible gateways:
                            # Some gateways can return finish_reason=stop with empty
                            # assistant content in non-streaming mode, while streaming
                            # still yields usable text deltas.
                            #
                            # Trigger a single silent streaming probe only when:
                            # - we are currently on non-streaming path
                            # - response_text is empty
                            # - endpoint is third-party OpenAI-compatible
                            # - feature flag is not disabled
                            _allow_nonstream_empty_recovery = str(
                                os.getenv("PRAISONAI_DISABLE_NONSTREAM_EMPTY_RECOVERY", "")
                            ).strip().lower() not in {"1", "true", "yes", "on"}
                            _response_empty_after_nonstream = not str(response_text or "").strip()
                            _effective_base = self._get_effective_openai_base_url()
                            _is_third_party_openai = bool(
                                _effective_base and not self._is_official_openai_base_url(_effective_base)
                            )
                            if (
                                _allow_nonstream_empty_recovery
                                and _response_empty_after_nonstream
                                and _is_third_party_openai
                            ):
                                logging.debug(
                                    "[NONSTREAM_EMPTY_RECOVERY] empty non-stream response detected; "
                                    "trying one silent streaming probe (model=%s, has_tools=%s)",
                                    self.model,
                                    bool(formatted_tools),
                                )
                                try:
                                    ...
                                    for chunk in litellm.completion(
                                        **self._build_completion_params(
                                        ...
                                        if chunk and chunk.choices and chunk.choices[0].delta:
                                            delta = chunk.choices[0].delta
                                            _probe_text, _probe_tool_calls = self._process_stream_delta(
                                                delta, _probe_text, _probe_tool_calls, formatted_tools
                                            )

                                    if str(_probe_text or "").strip() or _probe_tool_calls:
                                        response_text = _probe_text
                                        tool_calls = _probe_tool_calls
                                        final_response = {
                                            "choices": [{
                                                "message": {
                                                    "content": response_text,
                                        ...
                                        logging.debug(
                                            "[NONSTREAM_EMPTY_RECOVERY] recovered response via stream probe: "
                                            "text_len=%s tool_calls=%s",
                                            len(response_text or ""),
                                            bool(tool_calls),
                                        )
                                    else:
                                        logging.debug(
                                            "[NONSTREAM_EMPTY_RECOVERY] stream probe also returned empty content"
                                        )
                                except Exception as _probe_err:
                                    logging.debug(
                                        "[NONSTREAM_EMPTY_RECOVERY] stream probe failed: %s",
                                        _probe_err,
                                    )
                    ...
                    legacy_function_call = message_payload.get("function_call")
                    finish_reason = final_response["choices"][0].get("finish_reason")

                    logging.debug(
                        "[TOOL_DEBUG_SYNC] iteration=%s finish_reason=%s has_tool_calls=%s has_function_call=%s content_len=%s content_preview=%s",
                        iteration_count,
                        finish_reason,
                        bool(tool_calls),
                        bool(legacy_function_call),
                        len(response_text or ""),
                        (response_text or "")[:200],
                    )

                    # Deep diagnostics for empty assistant payloads on gateway-compatible APIs.
                    # Enable with PRAISONAI_EMPTY_RESPONSE_DIAG=1.
                    _diag_enabled = str(os.getenv("PRAISONAI_EMPTY_RESPONSE_DIAG", "")).strip().lower() in {"1", "true", "yes", "on"}
                    if _diag_enabled and (not (response_text or "").strip()) and not tool_calls:
                        try:
                            response_map = self._to_mapping(final_response)
                            choice0 = None
                            if isinstance(response_map, dict):
                                choices_raw = response_map.get("choices")
                                if isinstance(choices_raw, list) and choices_raw:
                                    choice0 = choices_raw[0]
                            if choice0 is None:
                                try:
                                    choices_raw = getattr(final_response, "choices", None)
                                    if isinstance(choices_raw, list) and choices_raw:
                                        choice0 = choices_raw[0]
                                except Exception:
                                    choice0 = None

                            choice0_map = self._to_mapping(choice0)
                            message_raw = None
                            if isinstance(choice0_map, dict):
                                message_raw = choice0_map.get("message")
                            if message_raw is None:
                                try:
                                    message_raw = getattr(choice0, "message", None)
                                except Exception:
                                    message_raw = None
                            message_map = self._to_mapping(message_raw)
                            choice_provider_fields = (
                                choice0_map.get("provider_specific_fields")
                                if isinstance(choice0_map, dict)
                                else None
                            )

                            choice_keys = list(choice0_map.keys()) if isinstance(choice0_map, dict) else []
                            msg_keys = list(message_map.keys()) if isinstance(message_map, dict) else []
                            provider_fields = message_map.get("provider_specific_fields") if isinstance(message_map, dict) else None
                            provider_keys = list(provider_fields.keys()) if isinstance(provider_fields, dict) else []
                            choice_provider_keys = list(choice_provider_fields.keys()) if isinstance(choice_provider_fields, dict) else []

                            # One more extraction attempt from raw payload (for field-shape drift).
                            alt_text = ""
                            for key in (
                                "content",
                                "text",
                                ...
                                if isinstance(message_map, dict):
                                    alt_text = self._coerce_content_to_text(message_map.get(key))
                                if not alt_text and isinstance(choice0_map, dict):
                                    alt_text = self._coerce_content_to_text(choice0_map.get(key))
                                if not alt_text and isinstance(choice_provider_fields, dict):
                                    alt_text = self._coerce_content_to_text(choice_provider_fields.get(key))
                                if not alt_text and message_raw is not None:
                                    try:
                                        alt_text = self._coerce_content_to_text(getattr(message_raw, key, None))
                                    except Exception:
                                        alt_text = ""
                                if alt_text:
                                    break
                            ...
                            logging.debug(
                                "[EMPTY_DIAG_SYNC] finish_reason=%s choice_type=%s message_type=%s choice_keys=%s message_keys=%s provider_keys=%s choice_provider_keys=%s alt_text_len=%s message_preview=%s choice_provider_preview=%s",
                                finish_reason,
                                type(choice0).__name__ if choice0 is not None else None,
                                type(message_raw).__name__ if message_raw is not None else None,
                                choice_keys,
                                msg_keys,
                                provider_keys,
                                choice_provider_keys,
                                len(alt_text or ""),
                                str(message_raw)[:500],
                                str(choice_provider_fields)[:500],
                            )
                            if alt_text and not response_text:
                                response_text = alt_text.strip()
                                logging.debug(
                                    "[EMPTY_DIAG_SYNC] Recovered response_text from alternative field extraction, len=%s",
                                    len(response_text),
                                )
                        except Exception as _diag_err:
                            logging.debug("[EMPTY_DIAG_SYNC] diagnostics failed: %s", _diag_err)
                    
                    
                    # Parse tool calls from plain JSON text when provider doesn't emit native tool_calls
                    # (common in some third-party OpenAI-compatible gateways)
                    if not tool_calls and response_text and formatted_tools:
                        parsed_tool_calls, parse_error = self._try_parse_tool_call_json(
                            response_text,
                            iteration_count,
                        )
                        if parsed_tool_calls:
                            ...
                            logging.debug(
                                "Parsed tool call(s) from response JSON text for provider fallback: %s",
                                tool_calls,
                            )
                        elif parse_error:
                            logging.debug(
                                "Could not parse JSON tool call from response text (non-fatal): %s",
                                parse_error,
                            )
            ...
            if not self._coerce_content_to_text(response_content):
                tool_summary = self._generate_tool_results_summary(accumulated_tool_results)
                response_content = tool_summary or self._get_empty_response_fallback_message()
                    ...
                    if tool_calls and execute_tool_fn:
                        ...
                        try:
                            ...
                            if follow_up_response and follow_up_response.choices:
                                follow_up_content = self._extract_text_from_completion_response(follow_up_response)
            ...
            if not use_streaming:
                ...
                try:
                    ...
                    if response and response.choices:
                        content = self._extract_text_from_completion_response(response)
                        if content:
        ...
        **kwargs
        ...
        try:
            ...
            while iteration_count < max_iterations:
                ...
                if reasoning_steps and iteration_count == 0:
                    ...
                    if use_streaming:
                        ...
                        # Handle None content from Gemini
                        message_payload = tool_response.choices[0].message
                        response_content = message_payload.get("content") if isinstance(message_payload, dict) else getattr(message_payload, "content", None)
                        response_text = self._extract_text_from_completion_response(tool_response)
                        tool_calls = message_payload.get("tool_calls", [])
                        legacy_function_call = message_payload.get("function_call")
                        finish_reason = None
                        try:
                            ...
                            finish_reason = getattr(tool_response.choices[0], "finish_reason", None)

                        logging.debug(
                            "[TOOL_DEBUG_ASYNC] iteration=%s finish_reason=%s has_tool_calls=%s has_function_call=%s content_len=%s content_preview=%s",
                            iteration_count,
                            finish_reason,
                            bool(tool_calls),
                            bool(legacy_function_call),
            ...
            if not self_reflect:
                ...
                if not self._coerce_content_to_text(display_text):
                    tool_summary = self._generate_tool_results_summary(accumulated_tool_results)
                    display_text = tool_summary or self._get_empty_response_fallback_message()
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
    ...
    def _build_completion_params(self, **override_params) -> Dict[str, Any]:
        ...
        params = {
            "model": self._get_model_for_litellm_completion(),
        }
        ...
        if self.extra_settings:
            ...
            internal_extra_keys = {
                'metrics',
                # Internal tool-orchestration controls (PraisonAI-side only)
                'force_tool_usage',
                'max_tool_repairs',
            }
            filtered_extra_settings = {
                k: v for k, v in self.extra_settings.items() if k not in internal_extra_keys
            }
        ...
        # Third-party OpenAI-compatible gateways may return empty assistant messages
        # when explicit `tool_choice: "auto"` is sent. Let provider default behavior
        # decide tool routing in that case.
        try:
            _force_explicit_auto = str(
                os.getenv("PRAISONAI_FORCE_EXPLICIT_TOOL_CHOICE_AUTO", "")
            ...
            if (
                not _force_explicit_auto
                and _is_third_party_openai
                and str(params.get("tool_choice", "")).strip().lower() == "auto"
            ):
                params.pop("tool_choice", None)
                logging.debug(
                    "[TOOL_CHOICE_COMPAT] Removed explicit tool_choice='auto' "
                    "for third-party OpenAI-compatible endpoint"
                )
        except Exception:
            pass
        ...
        internal_params = [
            ...
            # Internal tool-orchestration controls (PraisonAI-side only)
            'force_tool_usage', 'max_tool_repairs',
        ]
        ...
        if 'tools' in params and params['tools'] and 'tool_choice' not in params:
            try:
                ...
                if (not _is_third_party_openai or _force_explicit_auto) and litellm.supports_function_calling(model=self.model):
                    params['tool_choice'] = 'auto'
                    logging.debug(f"Set tool_choice='auto' for model {self.model}")
            except Exception as e:
                # If check fails, only set explicit auto for non-third-party gateways,
                # unless force-enabled via env.
                _force_explicit_auto = str(
                    os.getenv("PRAISONAI_FORCE_EXPLICIT_TOOL_CHOICE_AUTO", "")
                ).strip().lower() in {"1", "true", "yes", "on"}
                _effective_base = self._get_effective_openai_base_url()
                ...
                if not _is_third_party_openai or _force_explicit_auto:
                    logging.debug(f"Could not verify function calling support: {e}. Setting tool_choice='auto' anyway.")
                    params['tool_choice'] = 'auto'
                else:
                    logging.debug(
                        "[TOOL_CHOICE_COMPAT] Skip implicit tool_choice='auto' on third-party endpoint: %s",
                        e,
                    )
    ...
    def _supports_responses_api(self) -> bool:
        ...
        # For third-party OpenAI-compatible endpoints, prefer chat/completions path.
        # Many providers do not fully implement Responses API semantics.
        effective_base = self._get_effective_openai_base_url()
        if effective_base and not self._is_official_openai_base_url(effective_base):
            return False
    ...
    def _process_tool_calls_from_stream(self, delta, tool_calls: List[Dict]) -> List[Dict]:
        ...
        # Legacy compatibility: some providers stream `function_call` instead of `tool_calls`
    ...
    def _extract_tool_call_info(self, tool_call, is_ollama: bool = False) -> tuple:
        ...
        if isinstance(tool_call, dict):
            ...
            try:
                ...
                arguments = {}
                tool_call_id = getattr(tool_call, "id", f"tool_{id(tool_call)}")
            return function_name, arguments, tool_call_id
        ...
        **kwargs
        ...
        try:
            ...
            if stream:
                ...
                response_text = self._extract_text_from_completion_response(response)
                if not response_text:
                    for attempt in range(1, 3):
                        retry_response = self._completion_with_retry(**completion_params)
                        response_text = self._extract_text_from_completion_response(retry_response)
                        if response_text:
                            logging.warning(
                                "[EMPTY_RESPONSE_NO_TOOLS] Recovered non-empty response on retry %s/2",
                                attempt,
                            )
                            break
                        time.sleep(0.3 * attempt)
        ...
        **kwargs
        ...
        try:
            ...
            if stream:
                ...
                if not response_text:
                    for attempt in range(1, 3):
                        retry_response = await self._acompletion_with_retry(**completion_params)
                        response_text = self._extract_text_from_completion_response(retry_response)
                        if response_text:
                            logging.warning(
                                "[EMPTY_RESPONSE_NO_TOOLS_ASYNC] Recovered non-empty response on retry %s/2",
                                attempt,
                            )
                            break
    ...
    def _generate_tool_definition(self, function_or_name) -> Optional[Dict]:
        ...
        def _annotation_to_json_type(annotation: Any) -> str:
            """Map Python/typing/string annotations to JSON schema type."""
            if annotation is inspect.Parameter.empty or annotation is None:
                return "string"

            # Resolve typing aliases like Optional[int], List[str], Dict[str, Any]
            ...
            # PEP563 / future annotations: annotation can be a string like 'int'
            if isinstance(annotation, str):
                ann = annotation.strip().lower()
                if ann in {"int", "integer"}:
                    return "integer"
                if ann in {"float", "double", "number"}:
                    return "number"
                if ann in {"bool", "boolean"}:
                    return "boolean"
                if ann in {"list", "list[str]", "list[int]", "list[float]", "list[bool]"}:
                    return "array"
                if ann in {"dict", "dict[str, any]", "mapping", "object"}:
                    return "object"
                if ann.startswith(("optional[", "typing.optional[")) and ann.endswith("]"):
                    inner = ann.split("[", 1)[1][:-1]
                    return _annotation_to_json_type(inner)
                if ann.startswith(("list[", "typing.list[")):
                    return "array"
                if ann.startswith(("dict[", "typing.dict[")):
                    return "object"
